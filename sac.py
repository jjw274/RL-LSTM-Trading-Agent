...
class TradingEnv(gym.Env):
    ...
    def compute_reward(self, shares_sold, executed_price):
        ...
        actual_price = float(self.calculate_vwap(self.current_step, shares_sold))
...
# Train the and save the model
print("Training the SAC model...")
model.learn(total_timesteps=total_timesteps, callback=EpisodeSummaryCallback(total_timesteps), log_interval=10)
...
while not done:
    ...
    print(f"Reward: {reward}, Inventory: {env.inventory}")

print("Model evaluation completed.")

def generate_trade_schedule(model, env, episode_length=390):
    # Reset the environment and initialize the trade schedule list
    obs = env.reset()
    done = False
    trade_schedule = []
    current_minute = 0  # Start from the 0th minute

    while not done and current_minute < episode_length:
        ...
        # Record current minute as the timestamp and share size for the trade
        trade_event = {
            "Timestamp": current_minute,  # Record the current minute as the timestamp
            "Share Size": action[0]  # Predicted share size
        }
        trade_schedule.append(trade_event)
        
        current_minute += 1  # Increment the minute for each step taken

    # If the episode ends early, pad the remaining minutes with zero shares
    while current_minute < episode_length:
        trade_event = {
            "Timestamp": current_minute,
            "Share Size": 0.0  # Zero shares for remaining minutes
        }
        trade_schedule.append(trade_event)
        current_minute += 1

    # Convert the trade schedule to JSON format
    trade_schedule_json = json.dumps(trade_schedule, indent=4, default=str)
    return trade_schedule_json

# Run the function to generate and save the example schedule
trade_schedule_json = generate_trade_schedule(model, env)
print("Generated Trade Schedule:\n", trade_schedule_json)
with open("trade_schedule.json", "w") as f:
    f.write(trade_schedule_json)
...
def evaluate_model_over_days(model, env, benchmark, initial_inventory=1000, preferred_timeframe=390):
    ...
    for day_index, day_data in enumerate(env.sequences):
        ...
        while not done:
            ...
            steps += 1
            trade_event = {"Timestamp": env.day_data[env.current_step - 1][0], "Share Size": action[0]}
            trade_schedule.append(trade_event)
